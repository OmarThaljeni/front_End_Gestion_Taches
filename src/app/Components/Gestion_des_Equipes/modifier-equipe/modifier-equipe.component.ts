...
export class ModifierEquipeComponent implements OnInit {
  ...
  constructor(private projetService: GestionProjetService, private userService: GestionUsersService, private gestionEquipeService: GestionEquipeService, private notificationService: NotificationService,
    ...
    public dialogRef: MatDialogRef<ModifierEquipeComponent>) {
  ...
  form: FormGroup = new FormGroup({
    ...
    projetForm: new FormControl('', [Validators.required, Validators.maxLength(200)]),
  ...
  ModifierEquipe() {
    ...
    this.gestionEquipeService.ModifierEquipe(this.data.id,equipe).subscribe(res => {
      this.submitted = true
